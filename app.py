...
def create_vector_store():
    ...
    try:
        return FAISS(embedding_function=st.session_state.embeddings, index=faiss.IndexFlatL2(len(st.session_state.embeddings.embed_query("Hello World"))),
...
def add_documents_to_vectorstore(documents: List[str]):
    ...
    try:
        st.session_state.vector_store.add_texts(texts=documents)
    except Exception as e:
...
def create_retrieval():
    ...
    try:
        return create_retrieval_chain(st.session_state.retriver, create_question_chain())
    except Exception as e:
...
if "model" not in st.session_state:
    st.session_state["model"] = ChatOllama(
        model=config["base_model"], base_url=config["local_url"])
if "embeddings" not in st.session_state:
    st.session_state["embeddings"] = OllamaEmbeddings(
        model=config["embedding_model"], base_url=config["local_url"])
...
with st.sidebar:
    with st.spinner():
        uploaded_pdf = st.file_uploader(
            "Upload your document (pdf)", type="pdf")
...
if prompt := st.chat_input("Type here..."):
    ...
    with st.chat_message("assistant"):
        ...
        st.write({f"Resource": documents[0]["context"][0].page_content})
        documents = []
